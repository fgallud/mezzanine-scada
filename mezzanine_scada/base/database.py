...
class variable_database:
    ...
    def set_value(self,name='',value=0.0,t_aquisition=0.0):
        try:
            ...
            return {'value':float('nan'),'time':t_aquisition,'error':True}
        if t_aquisition==0.0:
        ...
        return {'value':value,'time':t_aquisition}
    
    def get_value(self,name=''):
        if name in self.data:
            self.access.acquire()
            try:
                tmp_value={'value':self.data[name]['value'], 'time':self.data[name]['time']}
            except:
                logging.error(self.name+' signal '+name+' does not exist')
                tmp_value={'value':float('nan'),'time':0.0}
            self.access.release()
            return tmp_value
        else:
    ...
    def get_multiple_value(self,names=[]):
        data={}
        for n in names:
            new_data=self.get_value(n)
            if not isnan(new_data['value']):
                data[n]=new_data
        return data
